      ...
      <Modal accessibleName={title} open={open} title={title} onClose={onClose}>
        ...
        >
          {({
            ...
            const showDataTypes =
              values.type === "literal" ||
              (values.type === "value" && values.value === "plain");
            const showRepeated =
              values.type === "internal" || values.type === "value";
...
const NodeSchema = Yup.object().shape({
  ...
  repeated: Yup.boolean().required(
    "Please select if this node should be repeated"
  ),
  dataType: Yup.string().when(["type", "value"], {
    is: (type, value) =>
      type === "literal" || (type === "value" && value === "plain"),
