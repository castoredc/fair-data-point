...
export default class AddNodeModal extends Component<AddNodeModalProps, AddNodeModalState> {
    ...
    handleSubmit = (values, {setSubmitting}) => {
        ...
        axios.post('/api/model/' + modelId + '/v/' + versionId + '/node/' + type + (data ? `/${data.id}` : ''), values)
            .then((response) => {
                setSubmitting(false);
            ...
            >
                {({
                    ...
                    return <Form>
                        ...
                        {values.type === 'external' && <FormItem label="URI">
                            <Field
                                component={Input}
                                name="value"
                                serverError={validation}
                        ...
                        {values.type === 'literal' && <FormItem label="Value">
                            <Field
                                component={Input}
                                name="value"
                                serverError={validation}
...
const NodeSchema = Yup.object().shape({
    ...
    value: Yup.string().required('Please enter a value').when('type', {
        is: 'external',
