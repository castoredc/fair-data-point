...
export default class DataDictionaryGroupModal extends Component {
    constructor(props) {
        super(props);
    ...
    handleSubmit = () => {
        const {data} = this.state;
        const {dictionaryId, versionId, onSaved} = this.props;
        ...
        if (this.form.isFormValid()) {
            ...
            axios.post('/api/dictionary/' + dictionaryId + '/v/' + versionId + '/group' + (data.id ? '/' + data.id : ''), data)
                .then(() => {
                    this.setState({
                        isSaved: true,
                        isLoading: false,
    ...
    handleDelete = (callback) => {
        ...
        if (data.id) {
            axios.delete('/api/dictionary/' + dictionaryId + '/v/' + versionId + '/group/' + data.id)
                .then(() => {
                    callback();
                    onSaved();
                })
    ...
    render() {
        ...
        return <Modal
            ...
            closeButton
            className={classNames('DataDictionaryGroupFormModal', data.dependent && 'ShowDependencyEditor')}
            footer={(
                <div className={classNames(data.id && 'HasConfirmButton')}>
                    ...
                    >
                        ...
                        >
                            ...
                            This will also delete all associated variables.
                        </ConfirmModal>}
