...
export default class NodeMappingInterface extends Component {
  constructor(props) {
    ...
    this.state = {
      selectedElements:
        props.mapping && props.mapping.elements ? props.mapping.elements : [],
      dataTransformation: props.mapping ? props.mapping.transformed : false,
      transformSyntax: props.mapping
        ? props.mapping.transformed
          ? props.mapping.syntax
          : ""
        : "",
      step: "elements",
    };
  ...
  handleSubmit = () => {
    const { selectedElements, dataTransformation, transformSyntax } =
      this.state;
  ...
  render() {
    ...
    const fieldDescription = (
      <span>{mapping.node.repeated && <b>repeated</b>} fields</span>
    );
    ...
    return (
      <>
        <Stack distribution="equalSpacing">
          <Heading type="Panel">
            {`${mapping.elements ? `Edit` : `Add`} mapping for ${
              mapping.node.title
            }`}
          </Heading>
