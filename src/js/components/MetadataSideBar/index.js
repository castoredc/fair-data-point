...
const MetadataSideBar = ({name, type, metadata}) => {
    ...
    return <div className="MetadataSideBar">
        {shouldDisplay('logo') && <div className="InformationLogo">
        ...
        >
            ...
            {shouldDisplay('issued') && <MetadataItem label="Issued" table>
                <Tooltip content={moment(metadata.issued).format('DD-MM-YYYY HH:mm:ss')}>
                    {moment(metadata.issued).format('DD-MM-YYYY')}
                </Tooltip>
            </MetadataItem>}
            {shouldDisplay('modified') && <MetadataItem label="Modified" table>
