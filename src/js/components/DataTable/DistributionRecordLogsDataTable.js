...
export default class DistributionRecordLogsDataTable extends Component {
  ...
  render() {
    const { logs, isLoadingLogs, hasLoadedLogs, pagination, selectedLog } =
      this.state;
    ...
    const selectedLogItemHasErrors =
      ...
      selectedLogItem.errors !== null &&
      Array.isArray(selectedLogItem.errors) &&
      selectedLogItem.errors.length > 0;
            ...
            {selectedLogItem !== null ? (
              <div>
                ...
                <FormItem label="Status">
                  <DistributionGenerationStatus
                    status={selectedLogItem.status}
                  />
                </FormItem>

                <FormItem label="Date and time">
                  {moment(selectedLogItem.createdAt).format(
                    "DD-MM-YYYY HH:mm:ss"
                  )}
                </FormItem>
                ...
                <FormItem label="Errors" className="ErrorLog">
                  {selectedLogItemHasErrors ? (
                    ...
                    <div className="NoResults">
                      No errors occurred while generating this record
                    </div>
                  )}
                </FormItem>
              </div>
