...
export default class DataModelMappingsDataTable extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (
      type !== prevProps.type ||
      versionId !== prevProps.versionId ||
      lastHandledMapping !== prevProps.lastHandledMapping
    ) {
      this.getMappings(type);
    }
  }
  ...
  render() {
    const { mappings, isLoadingMappings, hasLoadedMappings, pagination } =
      this.state;
    const { type } = this.props;
    ...
    if (type === "node") {
      rows = mappings.map((item) => {
        return {
          mapped: !item.elements ? (
            <IconCell icon={{ type: "errorCircledInverted" }} />
          ) : undefined,
          ...
          repeated: item.node.repeated ? (
            <IconCell icon={{ type: "tickSmall" }} />
          ) : undefined,
          ...(item.transformed && {
            mappedElement: (
              <CellText>
          ...
          ...(!item.transformed && {
            mappedElement: (
              <CellText>{item.elements ? item.elements[0].label : ""}</CellText>
            ),
          }),
        };
      });
      ...
      rows = mappings.map((item) => {
        return {
          ...
          mappedElement: (
            <CellText>{item.element ? item.element.label : ""}</CellText>
          ),
          mappedElementType: (
            <CellText>
              {item.element ? item.element.structureType : ""}
            </CellText>
          ),
        };
      });
