...
export default class StudyForm extends Component {
  ...
  handleSubmit = (event) => {
    ...
    if (this.form.isFormValid()) {
      apiClient
        .post("/api/study" + (study ? "/" + study.id : ""), newData)
        .then((response) => {
          this.setState({
            isSaved: true,
            study: response.data,
            ...
            <div>
              <FormItem label="Server">
                <Dropdown
                  ...
                  onChange={(e) => {
                    this.handleChange({
                      target: { name: "sourceServer", value: e.value },
                    });
                  }}
                  value={castorServers.filter(
                    ({ value }) => value === data.sourceServer
                  )}
