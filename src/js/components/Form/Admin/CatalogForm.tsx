  ...
  constructor(props) {
    ...
    this.state = {
      initialValues: props.catalog
        ? mergeData(defaultData, props.catalog)
        : defaultData,
      validation: {},
    ...
    apiClient
      ...
      .then((response) => {
        ...
        if (catalog) {
          ...
          history.push(
            "/dashboard/catalogs/" + response.data.slug + "/metadata"
          );
        }
      })
      .catch((error) => {
...
const CatalogSchema = Yup.object().shape({
  ...
  acceptSubmissions: Yup.boolean().required(
    "Please enter if this catalog accepts submissions"
  ),
  submissionAccessesData: Yup.boolean().when("acceptSubmissions", {
    is: true,
    then: Yup.boolean().required(
      "Please enter if the submission accesses data in the process"
    ),
  }),
});
