...
> {
  ...
  getMetadata = () => {
    ...
    apiClient
      ...
      .then((response) => {
        ...
        if (metadata != null) {
          if (
            metadata.estimatedStudyStartDate !== "" &&
            metadata.estimatedStudyStartDate !== null
          ) {
            metadata.estimatedStudyStartDate = new Date(
              metadata.estimatedStudyStartDate
            );
          }
          if (
            metadata.estimatedStudyCompletionDate !== "" &&
            metadata.estimatedStudyCompletionDate !== null
          ) {
            metadata.estimatedStudyCompletionDate = new Date(
              metadata.estimatedStudyCompletionDate
            );
          }
        }
  ...
  handleSubmit = (values, { setSubmitting }) => {
    ...
    apiClient
      .post(
        "/api/study/" + studyId + "/metadata" + (data.id ? "/" + data.id : ""),
        {
          ...
          estimatedStudyStartDate: values.estimatedStudyStartDate
            ? format(values.estimatedStudyStartDate, "yyyy-MM-dd")
            : null,
          estimatedStudyCompletionDate: values.estimatedStudyCompletionDate
            ? format(values.estimatedStudyCompletionDate, "yyyy-MM-dd")
            : null,
                ...
                <div className="FormContent">
                  ...
                  <FormGroup label="Study Description">
                    <FormItem label="Brief Summary">
                      <p>
                        Please include the objective, design, research question
                        and the population of your study.
                      </p>
                      <Field
                        component={Input}
                        multiline={true}
...
const StudyDetailsSchema = Yup.object().shape({
  ...
  estimatedEnrollment: Yup.number()
