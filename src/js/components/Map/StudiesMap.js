...
export default class StudiesMap extends Component {
    render() {
        ...
        return (
            <div className="DatasetMap">
                <Map center={getCenterFromDegrees(coordinates)} zoom={3} attributionControl={false}>
                    <TileLayer
                    ...
                    <MarkerClusterGroup>
                        {studies.map((study, index) => {
                            return (
                                <Marker key={index} position={[study.coordinates.lat, study.coordinates.long]}>
                                    <Popup>
