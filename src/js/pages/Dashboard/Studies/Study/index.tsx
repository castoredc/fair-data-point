...
export default class Study extends Component<StudyProps, StudyState> {
      ...
      .catch((error) => {
        ...
        if (
          ...
          toast.error(
            <ToastContent
              type="error"
              message="An error occurred while loading the study"
            />
          );
        }
      });
  ...
  render() {
    ...
    if (!isGranted("edit", study.permissions)) {
      return (
        <NoPermission text="You do not have permission to edit this study" />
      );
    }

    const title = study.hasMetadata
      ? localizedText(study.metadata.briefName, "en")
      : study.name;
