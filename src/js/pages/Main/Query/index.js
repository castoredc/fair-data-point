...
export default class Query extends Component {
  ...
  getPrefixes = () => {
    ...
    apiClient
      .get(
        "/api/model/" +
          distribution.dataModel.dataModel +
          "/v/" +
          distribution.dataModel.id +
          "/prefix"
      )
      .then((response) => {
      ...
      () => {
        ...
        if (distribution) {
          config["requestConfig"] = {
            endpoint:
              window.location.origin + distribution.relativeUrl + "/sparql",
          };
        }
  ...
  render() {
    ...
    let title =
      hasDistribution && !isLoading
        ? localizedText(distribution.metadata.title, "en")
        : "Query";
        ...
        <MainBody isLoading={isLoading} className="QueryComponent">
          <Split sizes={[40, 60]}>
            <div className="QueryTools">
              <div
                className={classNames("QueryEditor", !showEditor && "Hide")}
                id="query"
              />
              <Stack
                ...
                <div className="ResultCount">
                  {executedWithoutErrors && (
                    <>
                      ...
                      <strong>{(executionTime / 1000.0).toFixed(2)}</strong>{" "}
                      seconds
                    </>
                  )}
                </div>
