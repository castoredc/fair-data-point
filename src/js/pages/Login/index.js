...
export default class Login extends Component {
  ...
  getServers = () => {
    apiClient
      ...
      .then((response) => {
        ...
        const defaultServer = response.data.filter(
          (server) => server.default
        )[0].id;
        const serverIds = response.data.map((server) => server.id);
        const urlParamServer =
          typeof params.server !== "undefined" ? parseInt(params.server) : null;
        const serverLocked =
          typeof params.serverLocked !== "undefined"
            ? params.serverLocked === "1"
            : false;
        const view =
          typeof params.view !== "undefined" ? params.view : "generic";
        const selectedServer =
          urlParamServer !== null && serverIds.includes(urlParamServer)
            ? urlParamServer
            : defaultServer;
  ...
  render() {
    const { serverLocked, view, servers, selectedServer, catalog, isLoading } =
      this.state;
